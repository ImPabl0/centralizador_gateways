...
import BackgroundVideoSwitcher from "./BackgroundVideoSwitcher";
  ...
  useEffect(() => {
    const createPixPayment = async () => {
      try {
        ...
        console.log(`PIX gerado com sucesso via ${apiUsada}:`, data);
        setPixData(data);
        setApiUsada(apiUsada.toLowerCase());
      } catch (err) {
  ...
  const retryCreatePixPayment = async () => {
    try {
      ...
      try {
        ...
        console.warn("PayEvo falhou no retry, tentando BlackCat:", payevoError);
      ...
      const data = await response.json();
      console.log(`PIX gerado com sucesso no retry via ${apiUsada}:`, data);
      setPixData(data);
      setApiUsada(apiUsada.toLowerCase());
    } catch (err) {
      console.error("Erro ao criar pagamento PIX em ambas APIs no retry:", err);
      setError("Erro ao gerar código PIX. Tente novamente.");
    } finally {
      setLoading(false);
    }
